<?php
    ...
    class LoginController extends BaseController
    {
        ...
        public function sendMail()
        {
            ...
            if(M('forget_pass')->where(['email'=>['eq',$_POST['email']],'use'=>['eq',0]])->find()){
                if(M('forget_pass')->where(['email'=>['eq',$_POST['email']],'use'=>['eq',0]])->save($data)){
                    ...
                    if(send_mail($_POST['email'],'点击完成认证', '点击<a href="www.rr.com/Home/Login/resetPass/email/'.$a.'">链接</a>跳转或则直接访问www.rr.com/Home/Login/resetPass/email/'.$a.'<br>链接两小时内有效')){
        ...
        public function resetPass($email)
        {
            ...
            $fp = M('forget_pass')->where(['email'=>['eq',$e]])->find();
            if ($fp) {
